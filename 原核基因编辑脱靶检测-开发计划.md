# ProkDiff：原核基因编辑全基因组脱靶检测（Rust）开发计划

> **给后续实现（人或 agent）：** 在独立仓库按阶段实现。本文件是完整规格；不要在 `News-Rust-bioinformation` 目录仓库里建 crate。推荐按任务勾选推进，每阶段先写测试再写实现。
>
> 副标题（README / 论文一行）：**Prokaryotic genome diff for gene-editing QC — starter vs edited WGS**

**Goal：** 做一款面向原核编辑 QC 的全基因组非预期变异检测器：出发株与编辑株都做 WGS；NCBI 参考只作坐标骨架；差分后的突变按三类分类。引擎对标 breseq consensus，语义对标 Widney/Copley 2024。

**Architecture：** 外部调用 Bowtie2（不重写比对器）；Rust 实现 RA / MC / JC 的克隆 consensus 子集，以钉死版本的 breseq `.gd` 为 oracle。产品层对两株各自调用后做集合差，掩码目的编辑，再按 (3)→(1)→(2) 打标签。第一期也可走风险缓冲：先调 breseq 出 GD，Rust 只做差分与分类。

**Tech Stack：** Rust 2021 workspace；`noodles` 或 `rust-htslib` 读 BAM；系统安装 `bowtie2`；开发期钉死 **breseq 0.40.x** + 配套 bowtie2；输出 GD 兼容子集 + TSV + 摘要。不依赖 GPU，不重写 HMMER / ESM。
...
| 检索 | 机器学习里已有 **ProDiff**（无 k）。拼写必须带 k。建仓前再搜 crates.io / GitHub；若占用则 `prokdiff-qc` |

## Global Constraints

- 第一期仅 Illumina 短读；仅克隆 consensus。不做混群 polymorphism、长读长、GUIDE-seq。
- CLI **必须同时要出发株与编辑株 FASTQ**。仅编辑株 + NCBI 拒绝或强警告后退出。NCBI / 近缘基因组只作坐标骨架，不是生物学对照。
- 非预期突变 = `M_edited \ M_parent \ {intended}`（坐标与等位基因匹配后减去）。
...
- 引擎清洁室：按 [breseq methods](https://breseq.barricklab.org/0.40.1/methods/) 实现，**不复制 breseq 源码**（GPL-2.0）。对拍用 `.gd` 与 `gdtools SUBTRACT`。
...
**路径 A（第一期，与 Widney 一致）：**

1. 出发株、编辑株都比对到同一骨架参考。
2. 各自 consensus 得到突变集 `M_parent`、`M_edited`。
3. 非预期 = `M_edited \ M_parent \ {intended}`。

**路径 B（第二期可选）：** 先用出发株 consensus 改写参考，再只对编辑株调用。列表更干净，但注释坐标要随改写走，第一期不做。
...
flowchart LR
  ...
  subgraph engine [Engine_breseq_class]
...
- **比对：** 调用系统 `bowtie2`，Rust 用 `noodles` / `rust-htslib` 读 BAM。
...
**引擎实现路径（锁定推荐 B，A 为风险缓冲）：**

- **路径 B（计划正文）：** Rust 实现 RA / MC / JC，Bowtie2 外部调用。
- **路径 A（工期紧时）：** 调系统 `breseq` 出两个 `.gd`，Rust 只做 `SUBTRACT` + 三类分类 + 报告。可先交付产品层，再替换引擎。

## 4. 独立仓库布局（在服务器上新建，不要写进目录仓库）

```text
prokdiff/
  Cargo.toml                 # workspace
  crates/prokdiff/           # CLI
  crates/prokdiff_evidence/  # RA / MC / JC
  crates/prokdiff_classify/  # 出发株减去 + 目的掩码 + 三类标签
  crates/prokdiff_report/    # GD 子集 + TSV + 摘要
  testdata/VERSIONS.txt      # breseq / bowtie2 精确版本
  testdata/generate.sh       # 层 1 合成 FASTQ 可复现生成
  testdata/private/          # gitignore：用户配对 WGS
  docs/parity.md             # 与 breseq 允许 / 不允许的差异
  docs/schema.md             # 本文件第 5 节的落地副本
```

| crate | 职责 |
| --- | --- |
| `prokdiff` | CLI：`--starter` / `--edited` 必填；`--ref`；`--intended`；`--editor cas9\|cas12a\|dsb`；`--spacer` / `--pam`；`--outdir` |
| `prokdiff_evidence` | pileup 共识、覆盖缺口、junction；写出与 breseq 可对拍的 GD 子集 |
| `prokdiff_classify` | `subtract(edited, starter)`、目的掩码、近同源扫描、三类标签 |
| `prokdiff_report` | `unintended.tsv`、摘要、可选 HTML |
...
- `--starter` / `--edited`：可重复（R1/R2 或多 lane）。缺任一则退出，错误信息写明必须提供出发株 WGS。
- `--ref`：可重复。染色体 + 质粒 + donor 骨架都进同一套参考。
- `--intended`：目的编辑表（见下）。坐标相对骨架参考。
...
再减去 intended，与 `unintended.tsv` 的变异集合一致（允许 `docs/parity.md` 中列出的规范化差异）。
...
另写 `summary.txt`（或 JSON）：目的编辑是否观察到；三类计数；出发株相对骨架的变异总数（仅供质控，不列入非预期）。
...
5. **差分（强制）：** `M_edited` 减去 `M_parent`，再减去 intended。
6. **分类：**
...
原则：**breseq 是突变发现引擎的 oracle**（同一参考、同一 FASTQ、钉死版本）。产品层（出发株差分、三类分类）用合成双样本，不要求 breseq 自己会做「两株相减」。比对器也钉死，否则 RA/JC 会对不齐。
...
用参考 FASTA（可截一段大肠 ~200 kb 以加快 CI）+ `gdtools APPLY` 或等价脚本写入已知突变，再用 ART / InSilicoSeq 模拟 Illumina（定种子）。

| 夹具 | 做法 | 通过标准 |
| --- | --- | --- |
| `synth_snp_indel` | 参考上 5 个 SNP + 2 个短 indel | 与同数据 breseq `.gd` 的 SNP/INS/DEL 集合一致 |
| `synth_is_mob` | 模拟 IS 插入（参考里已有 IS 拷贝） | MOB/JC 不漏报；允许 junction 坐标 ± 数 bp 的规范化说明 |
...
### 层 2 — 官方真实数据（本地 / 发布前跑，CI 可选 `slow`）

1. **Test Drive**
   参考 `NC_012967`（REL606）；reads [SRR030257](https://www.ebi.ac.uk/ena/browser/view/SRR030257)（LTEE 2 万代，Barrick 2009）。命令：`breseq -r NC_012967.gbk SRR030257_*.fastq.gz`。说明：单样本相对祖先参考，**不是**出发株 vs 编辑株；只测引擎。
2. **Methods Mol. Biol. 2014 教程包**（[Barrick 下载页](https://barricklab.org/twiki/bin/view/Lab/ToolsBacterialGenomeResequencing)）
   `Clonal_Sample.tgz`（~267 MB 输入）+ `Clonal_Output.tgz`（期望输出）。优先把 `output.gd` 当金标准。
...
已有 breseq 结果的「出发株 + 编辑株」各跑一次，导出两个 `.gd`，`gdtools SUBTRACT edited.gd starter.gd` 作为产品层金标准。数据留在 `testdata/private/`（gitignore）。

### 与 breseq 一致的判定

允许的差异（写入 `docs/parity.md`，超出即失败）：

- UN / 未知区、HTML、R 图、BAM 文件名。
- JC 在重复区上的「哪一个拷贝」歧义（breseq 也有 unassigned JC）。
- 频率字段在 consensus 下的舍入。

不允许：SNP 位点 / 等位基因不一致；明确的 unique-mapping MOB / 大 DEL 漏报。

### CI

- PR：层 0 + 小合成 FASTQ（秒–分钟）。
- nightly / `cargo test --ignored`：层 1 全套 + 可选 subsample 的 SRR。
- 发版前：层 2 Test Drive 或 Clonal_Output 全量；记录 breseq 与 bowtie2 版本哈希。

## 8. 分阶段任务（在独立仓库勾选）

### 阶段 0 — 仓库脚手架

- [ ] 新建 `prokdiff` 仓库与 Cargo workspace（四个 crate）。
- [ ] `testdata/VERSIONS.txt` 写入本机 `breseq --version` 与 `bowtie2 --version`。
- [ ] `testdata/private/` 加入 `.gitignore`。
- [ ] README：副标题、必须双样本、三类定义、与 GUIDE-seq 的区别。

### 阶段 1 — 证据类型与 GD I/O（层 0）

- [ ] GD 读写：SNP/INS/DEL/MOB/JC 最小子集。
- [ ] 实现与 `gdtools SUBTRACT` 一致的集合差（坐标 + 类型 + 等位基因）。
- [ ] pileup 共识单元测试：SNP、1–2 bp indel、链偏倚过滤。
- [ ] JC 接受 / 拒绝规则的单元测试（无 FASTQ）。

### 阶段 2 — 引擎 MVP 或路径 A 缓冲

**推荐路径 B：**

- [ ] 包装 Bowtie2：两样本分别比对到同一 `--ref` 集合，产出 BAM。
...
- [ ] 层 1 `synth_snp_indel` 与同机 breseq `.gd` 双向 SUBTRACT 为空（在 parity 允许范围内）。

**路径 A（可先做，再换 B）：**

- [ ] `prokdiff` 调 `breseq` 两次，收集 `output.gd`。
- [ ] 后续阶段只依赖 GD，不依赖内部 pileup。
...
- [ ] 层 2：Test Drive 或 Clonal_Output；Example 6 cassette。
- [ ] 层 3：用户私有配对（若有）。
- [ ] 同机计时：先测再写 README，不在规格里写死「比 breseq 快 10×」。
...
- 同机同数据，RA 步明显快于 breseq 或总时间可接受（先测时再定倍数）。

## 10. 本目录仓库的说明

本文件保存在 `News-Rust-bioinformation` 根目录，与 [微生物基因编辑与基因组-值得用Rust做的方向.md](微生物基因编辑与基因组-值得用Rust做的方向.md) 并列，便于拷到服务器。

**不要在本目录仓库实现 ProkDiff。** 在服务器上另建 `prokdiff` 仓库，把本 Markdown 放进该仓库 `docs/` 即可开工。
